                ...
                @if (old('answer.keys'))
                    @foreach (old('answer.keys') as $key)
                        @if (old('answer.quiz_question_id.'.$key) == $dataId)
                            @include('admin.module.quizzes-package.answers::back.partials.block', ['key' => $key, 'quizType' => $quizType])
                        @endif
                    @endforeach
                @else
                    @if ($item->answers)
                        @foreach ($item->answers as $answer)
                            @include('admin.module.quizzes-package.answers::back.partials.block', ['item' => $answer, 'quizType' => $quizType])
                        @endforeach
                    @endif
                @endif
