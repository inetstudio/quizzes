...
@section('content')
    ...
    @push('breadcrumbs')
        @include('admin.module.quizzes-package.quizzes::back.partials.breadcrumbs.form')
    @endpush

    <div class="wrapper wrapper-content">
        <div class="ibox">
            <div class="ibox-title">
                <a class="btn btn-sm btn-white" href="{{ route('back.quizzes-package.quizzes.index') }}">
        ...
        {!! Form::open(['url' => (! $item->id) ? route('back.quizzes-package.quizzes.store') : route('back.quizzes-package.quizzes.update', [$item->id]), 'id' => 'mainForm', 'enctype' => 'multipart/form-data', 'class' => 'quizz-form']) !!}
            ...
            <div class="ibox">
                <div class="ibox-title">
                    {!! Form::buttons('', '', ['back' => 'back.quizzes-package.quizzes.index']) !!}
                </div>
                                        ...
                                        <div class="panel-body">
                                            ...
                                            @if (old('result.keys'))
                                                @foreach (old('result.keys') as $key)
                                                    @include('admin.module.quizzes-package.results::back.partials.block', ['key' => $key, 'quizType' => $quizType])
                                                @endforeach
                                            @else
                                                @foreach ($item->results as $result)
                                                    @include('admin.module.quizzes-package.results::back.partials.block', ['item' => $result, 'quizType' => $quizType])
                                                @endforeach
                                            @endif
                                        ...
                                        <div class="panel-body">
                                            ...
                                            @if (old('question.keys'))
                                                @foreach (old('question.keys') as $key)
                                                    @include('admin.module.quizzes-package.questions::back.partials.block', ['key' => $key, 'quizType' => $quizType])
                                                @endforeach
                                            @else
                                                @foreach ($item->questions as $question)
                                                    @include('admin.module.quizzes-package.questions::back.partials.block', ['item' => $question, 'quizType' => $quizType])
                                                @endforeach
                                            @endif
